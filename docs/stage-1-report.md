...
Перевірка виконана на гілках Neon, зроблених від `production`.

> ### ⚠️ Уточнення від 12.09.2026: міграції вже на production
>
> Це твердження раніше звучало як «прод не змінювався». Станом на
> **12.09.2026 10:25** воно **неправдиве**: `_prisma_migrations` на гілці
> `production` містить усі три міграції етапу 1 —
> `00000000000000_enable_extensions`, `20260911010000_seat_unique_per_tour`,
> `20260911020000_tourist_identity`.
>
> Фактичний стан схеми на проді звірено окремо (read-only):
> `booking_tourists.tour_id` існує й `NOT NULL`; індекс
> `booking_tourists_tour_id_bus_seat_number_key` є; старих
> `booking_tourists_booking_id_bus_seat_number_key` і `tourists_email_key`
> **немає**. Тобто зміна не просто зареєстрована, а реально застосована.
>
> Хто накотив — з журналу не видно. Це суперечить правилу завдання
> «не запускати міграції на production-БД, тільки локально та в CI»
> (над репо паралельно працює кілька сесій, CLAUDE.md §13).
>
> **Наслідок для мержу:** розгортати на проді вже нічого. Натомість
> зʼявляється питання відкату, якого раніше не було: зворотної міграції
> немає, а `tourists_email_key` уже знято.
...
## Передмержевий прогін (12.09.2026)

Виконано на окремих одноразових гілках Neon від `production`, з копією даних.
`main`/`production` як ціль запису не використовувались.

Вихідні дані на копії: 7 турів, 9 бронювань, 6 учасників, 10 туристів,
2 готелі, 26 таблиць.

### 1. `migrate reset --force` — падає, як і передбачав звіт

Гілка `stage-1-premerge`:

```
Applying migration `00000000000000_enable_extensions`   ← OK
Applying migration `20260705101941_add_tour_checklists`
Error: P3018 · 42P01 · relation "tours" does not exist
```

Підтверджено емпірично: baseline-міграції немає, базові 24 таблиці заведено
через `db push`. Накат з нуля неможливий — це блокер етапу 2, не регресія
етапу 1. Разом з тим видно, що доданий `00000000000000_enable_extensions`
свою роль виконує: розширення створюється міграцією й на ньому вже не падає.

### 2. `migrate deploy` на копії production

Гілка `stage-1-premerge-deploy`: `No pending migrations to apply` — бо всі
дев'ять уже зареєстровані (див. попередження про прод вище).

### 3. Інтеграційні тести — 5/5 на живій PostgreSQL

```
✓ test/integration/seat-uniqueness.test.ts    (4)
✓ test/integration/seats.concurrency.test.ts  (1)
  ✓ BR-01 · два паралельні claim на 1 місце: рівно один виграє, availableSeats = 0
Tests  5 passed (5)
```

### 4. Доказ, що конкурентний тест не порожній

Зеленого тесту замало: треба показати, що він ловить саме той баг. На тій
самій БД відтворено **стару** реалізацію (`findFirst` → `update`) з двома
незалежними зʼєднаннями й бар'єром, який гарантує перетин читань:

| Реалізація | Результат | `availableSeats` |
|---|---|---|
| стара (`findFirst` + `update`) | `ПРОЙШЛО` \| `ПРОЙШЛО` | **−1** — овербукінг |
| нова (`claimSeats` / `updateMany`) | `ВІДМОВЛЕНО` \| `ПРОЙШЛО` | **0** |

Овербукінг відтворюється лише на **прямому** (не pooled) зʼєднанні й двох
різних клієнтах: через pooler транзакції серіалізуються самі, і баг ховається.
Це пояснює, чому він не виявлявся раніше.

Побічно: бар'єрний сценарій до нової реалізації **незастосовний** — вона бере
блокування рядка вже на `updateMany`, тож друга транзакція чекає й до бар'єра
не доходить (дедлок за побудовою тесту, не дефект коду). Саме це блокування
й робить правку коректною.
